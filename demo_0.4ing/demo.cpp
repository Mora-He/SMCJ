...
void disp_to_depth(Mat disp, Mat* depth)
{
	...
	Mat temp(disp.rows, disp.cols, CV_8UC1);
	uchar* depthData = (uchar*)temp.data;
	...
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
		{
			...
			depthData[id] = uchar((float)fx*baseline / (float)dispData[id]);
		}
	}
	...
	*depth = temp;
	...
	//rectangle(rec, Point(0,0), Point(100,100), Scalar(0, 255, 0), 1, LINE_8, 0);

	Mat cann, temp2;
	Canny(*depth, cann, 6, 12);	// 边缘
	temp2 = Mat::zeros(cann.size().height + 2, cann.size().width + 2, cann.type());
	cann.copyTo(temp2(Range(1, cann.size().height + 1), Range(1, cann.size().width + 1)));
	floodFill(temp2, Point(0, 0), Scalar(255));

	Mat cutImg;
	temp2(Range(1, cann.size().height + 1), Range(1, cann.size().width + 1)).copyTo(cutImg);
	rec = cann | (~cutImg);
