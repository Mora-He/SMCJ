...
void UpdateObjOsd(RECT_S pstObjRect)
{
	HI_U32 iRet;
	memset(s_stDemoMngCtx.stOsdBitmap.pData, 0,
		   s_stDemoMngCtx.stOsdBitmap.u32Width * s_stDemoMngCtx.stOsdBitmap.u32Height * 2);
	DrawRectForArgb1555PackedPic(&s_stDemoMngCtx.stOsdBitmap, &pstObjRect, 2, 0x801f);
	iRet = HI_MPI_RGN_SetBitMap(s_stDemoMngCtx.hOsdRgn, &s_stDemoMngCtx.stOsdBitmap);
}
...
int main(int argc, char *argv[])
{
	...
	VIDEO_FRAME_INFO_S left;
	VIDEO_FRAME_INFO_S right;

	// result frame
	VIDEO_FRAME_INFO_S result;
	...
	// buzz = fopen(file_name, "w");
	// if (buzz == NULL)
	// {
	// 	printf("Cannot open %s.\n", file_name);
	// 	goto EXIT;
	// }
	// fprintf(buzz, "%d", gpio_num);
	// fclose(buzz);
	// sprintf(file_name, "/sys/class/gpio/gpio%d/direction", gpio_num);
	// buzz = fopen(file_name, "rb+");
	// if (buzz == NULL)
	// {
	// 	printf("Cannot open %s.\n", file_name);
	// 	goto EXIT;
	...
	{
		// get left frame
		// printf("mark get left\n");
		iRet = HI_MPI_VPSS_GetChnFrame(0, 1, &left, 300);
		if (HI_SUCCESS != iRet)
		{
			continue;
		}
		// get right frame
		// printf("mark get right\n");
		iRet = HI_MPI_VPSS_GetChnFrame(1, 1, &right, 300);
		if (HI_SUCCESS != iRet)
		{
			continue;
		}
		...
		iRet = SendPic(&matcher, &left, &right);
		printf("distance:%d\n", iRet);
		if (iRet < 10)
		{
			// strcpy(buf, "1");
		...
		// fwrite(buf, sizeof(char), sizeof(buf) - 1, buzz);
		// printf("%s: gpio = %s\n", __func__, buf);

		// // printf("mark sendpic end\n");
		rectangle.s32X = matcher.sy * VIDEO_INPUT_WIDTH / right.stVFrame.u32Width;
		rectangle.s32Y = matcher.sx * VIDEO_INPUT_HEIGHT / right.stVFrame.u32Height;
		rectangle.u32Width = (matcher.ey - matcher.sy) * VIDEO_INPUT_WIDTH / right.stVFrame.u32Width;
		rectangle.u32Height = (matcher.ex - matcher.sx) * VIDEO_INPUT_HEIGHT / right.stVFrame.u32Height;
		HI_MPI_VPSS_ReleaseChnFrame(0, 1, &left);
		HI_MPI_VPSS_ReleaseChnFrame(1, 1, &right);
		// printf("mark update obj begin\n");
		...
		// }
		// printf("mark\n");
		if (rectangle.u32Width != 0 && rectangle.u32Height != 0)
		{
			// printf("Update\n");
			UpdateObjOsd(rectangle);
		}
	}
	StopObjOsd();
	// close(fd);
